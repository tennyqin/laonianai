...
public class CountryEligibilityService {
    private static final Set<String> HAINAN_COUNTRIES=Set.of("albania","argentina","australia","austria","belarus","belgium","bosnia","brazil","brunei","bulgaria","canada","chile","croatia","cyprus","czechia","denmark","estonia","finland","france","germany","greece","hungary","iceland","indonesia","ireland","italy","japan","kazakhstan","kyrgyzstan","latvia","lithuania","luxembourg","malaysia","malta","mexico","monaco","montenegro","netherlands","newzealand","northmacedonia","norway","philippines","poland","portugal","qatar","korea","romania","russia","serbia","singapore","slovakia","slovenia","spain","sweden","switzerland","thailand","ukraine","uae","uk","usa","vietnam");
    private static final Set<String> TRANSIT_240_COUNTRIES=Set.of("albania","austria","belarus","belgium","bosnia","bulgaria","croatia","cyprus","czechia","denmark","estonia","finland","france","germany","greece","hungary","iceland","ireland","italy","latvia","lithuania","luxembourg","malta","monaco","montenegro","netherlands","northmacedonia","norway","poland","portugal","romania","russia","serbia","slovakia","slovenia","spain","sweden","switzerland","ukraine","uk","canada","usa","argentina","brazil","chile","mexico","australia","newzealand","brunei","indonesia","japan","kyrgyzstan","qatar","singapore","korea","uae","vietnam");
    ...
    public CountryEligibilityService(CountryCodeResolver resolver){this.resolver=resolver;}

    public Map<String,Object> build(String countryCode,List<CountryDetail> policies,Map<String,Object> extra){
        String normalized=resolver.policyKey(countryCode);List<Map<String,Object>> rules=buildPolicies(policies);
        if(CURRENT_UNILATERAL_30.contains(normalized)&&rules.stream().noneMatch(p->"unilateral".equals(p.get("type")))){Map<String,Object>p=new LinkedHashMap<>();p.put("type","unilateral");p.put("stayDays",30);p.put("purposes",List.of("tourism","business","family","exchange","transit"));p.put("requiresOnward",false);p.put("ordinaryPassportOnly",true);p.put("sourceRule","Ordinary passport holders may enter visa-free for business, tourism, visits to relatives and friends, exchange visits or transit, for up to 30 days.");rules.add(p);}
        Map<String,Object>root=new LinkedHashMap<>();root.put("version","2026-08-20");root.put("passportTypes",List.of("ordinary"));root.put("ordinaryPassportOnly",true);root.put("policies",rules);
        ...
        root.put("transit24",Map.of("enabled",true,"maxStayHours",24,"requiresConfirmedOnwardTicket",true,"restrictedAreaOnly",true,"officialSource","https://en.nia.gov.cn/n147418/n147463/c183412/content.html"));root.put("priority",extra==null?999:number(extra.get("priority"),999));return root;
    }
