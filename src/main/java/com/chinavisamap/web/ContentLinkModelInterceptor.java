...
public class ContentLinkModelInterceptor implements HandlerInterceptor {
    ...
    private void loadCountryNames(ObjectMapper mapper, String fileName) {
        try {
            ...
            // Optional recommendation data must never break page rendering.
        }
    }
            ...
            if("country-home".equals(mav.getViewName())){
                ...
                if(priority!=null){
                    ...
                    if(profileObject instanceof Map){
                        ...
                        p.put("priority",priority.getOrDefault("priority",p.get("priority")));
                        p.put("tier",priority.getOrDefault("tier",p.get("tier")));
                    ...
                    // Country-extra is the canonical personalized source when there is no
                    // priority-country-content record; keep its related links validated too.
    ...
    private List<String> normalizeRelatedCodes(List<?> raw,String currentCode){
        ...
        for(Object item:raw){
            String value=String.valueOf(item).trim(); if(value.isEmpty())continue;
            String canonical=resolver.routeCode(resolver.policyKey(value));
            if(canonical.isEmpty()||canonical.equals(currentCode)||!countryNames.containsKey(canonical))continue;
    ...
    /** Deterministic policy -> article links. */
    private List<Map<String,Object>> relatedArticles(String code,String type){
        return articles.stream().filter(a->belongsToCountry(a,code)).sorted((a,b)->{
            int byScore=Integer.compare(score(b,code,type),score(a,code,type)); if(byScore!=0)return byScore;
            String bt=String.valueOf(b.getOrDefault("publishAt","")),at=String.valueOf(a.getOrDefault("publishAt",""));
            int byDate=bt.compareTo(at); if(byDate!=0)return byDate;
    ...
    private boolean belongsToCountry(Map<String,Object> article,String code){
        Object raw=article.get("relatedCountryCodes"); if(!(raw instanceof List))return false;
        for(Object item:(List<?>)raw){String candidate=resolver.routeCode(resolver.policyKey(String.valueOf(item)));if(candidate.equals(code))return true;}
        return false;
    }

    private int score(Map<String,Object>a,String code,String type){
        int score=0; String category=String.valueOf(a.getOrDefault("categoryEn",""));
        String text=(String.valueOf(a.getOrDefault("titleEn",""))+" "+String.valueOf(a.getOrDefault("titleZh",""))+" "+String.valueOf(a.getOrDefault("summaryEn",""))+" "+String.valueOf(a.getOrDefault("summaryZh",""))).toLowerCase(Locale.ROOT);
        if("transit".equals(type)){if(category.toLowerCase(Locale.ROOT).contains("visa"))score+=30;if(containsTag(a,"Transit"))score+=35;if(containsTag(a,"240-hour"))score+=30;if(text.contains("transit")||text.contains("过境"))score+=20;}
        else if("unilateral".equals(type)||"mutual".equals(type)){if(category.toLowerCase(Locale.ROOT).contains("visa-free"))score+=20;if(containsTag(a,"Visa-Free"))score+=25;if(text.contains("visa-free")||text.contains("免签"))score+=15;}
        Map<String,Object> priority=priorityContent.get(resolver.policyKey(code));
        if(priority!=null){score+=Math.max(0,20-number(priority.get("priority"),999)/10);}
        return score;
    }
    ...
    private List<String> relatedCountries(Map<String,Object>a){
        Object raw=a.get("relatedCountryCodes"); if(!(raw instanceof List))return Collections.emptyList();
        return normalizeRelatedCodes((List<?>)raw,"");
    }
